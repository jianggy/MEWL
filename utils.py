...
def load_materials(material_dir):
    ...
    bpy.ops.wm.append(filename=os.path.join(
        material_dir, 'Shadeless.blend', 'Material', 'Shadeless'))
...
def draw_location(args, mask: np.ndarray, r, small_r, canvas_grid_size,
                  canvas_actual_size, phase_output_object_mask=False):
    y, x = np.where(mask != 0)
    ...
    coor = ((y[i] / canvas_grid_size - 0.5) * canvas_actual_size,
            (x[i] / canvas_grid_size - 0.5) * canvas_actual_size)

    # masked_area = int(obj_hw * (1 - overlapping_ratio))

    # upper_left = (max(y[i] - masked_area, 0), max(x[i] - masked_area, 0))
    # lower_right = (min(y[i] + masked_area, canvas_hw - 1),
    #                min(x[i] + masked_area, canvas_hw - 1))

    # rr, cc = skimage.draw.rectangle(start=upper_left, end=lower_right)

    # args.min_dist * 100 since we use 600 segment to approximate 6
    rr, cc = skimage.draw.ellipse(y[i], x[i], 
        (args.min_dist + r + small_r) * canvas_grid_size / canvas_actual_size, 
        (args.min_dist + r + small_r) * canvas_grid_size / canvas_actual_size,
        shape=mask.shape)

    if phase_output_object_mask:
